        ...
        <div className="space-y-0.5">
            {lines.map((line, i) => {
                ...
                if (/^#{1,3}\s/.test(trimmed)) {
                    return <span key={i} className="block text-[12px] font-bold text-[var(--chat-fg-strong)] mt-2 mb-0.5">{trimmed.replace(/^#+\s*/, "")}</span>;
                }
                if (/^\*\*.+\*\*$/.test(trimmed)) {
                    return <span key={i} className="block text-[12px] font-bold text-[var(--chat-fg-strong)] mt-1">{trimmed.replace(/^\*\*|\*\*$/g, "")}</span>;
                }
                if (/^```/.test(trimmed)) return null;
                if (/^[-•]\s/.test(trimmed)) {
                    return (
                        <span key={i} className="flex gap-2 items-start">
                            <span className="mt-[5px] h-1 w-1 rounded-full bg-[var(--chat-fg-dim)] shrink-0" />
                            <span className="text-[var(--chat-fg)]"><InlineFormatted text={trimmed.replace(/^[-•]\s*/, "")} /></span>
                        </span>
                    );
                }
...
export function Chatbot() {
    ...
    const chatVars = (isBrut
        ...
        : {
              ...
              "--chat-fg-strong": "rgba(255,255,255,0.9)",
              "--chat-fg-dim": "rgba(255,255,255,0.3)",
                ...
                {isOpen && (
                    <motion.div
                        ...
                        className={cn(
                            ...
                            "overflow-hidden",
                            isBrut ? "rounded-t-[8px] sm:rounded-[8px]" : "rounded-t-3xl sm:rounded-2xl"
                        )}
                        style={isBrut ? {
                            ...chatVars,
                    ...
                    >
                        ...
                        <div className="relative shrink-0 px-5 pt-5 pb-4"
                            ...
                            <motion.div className="absolute top-0 left-0 right-0 h-[1.5px]"
                                style={{ background: `linear-gradient(90deg, transparent, ${accentPrimary}90, transparent)` }}
                        ...
                        {/* ─── Divider ─── */}
                        <div className="mx-5" style={{ height: isBrut ? 2 : 1, background: "var(--chat-line)" }} />
                            ...
                            {visibleMessages.map((msg) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isAssistant ? (
                                            <div className="max-w-[88%] text-[12.5px] leading-[1.75] text-[var(--chat-fg)]"
                                                style={{ letterSpacing: "0.01em" }}>
                                                {clean ? (() => {
                                                    const cards = parseMessageToCards(clean);
                                                    const hasCards = cards.some((c) => c.type !== "text");
                                                    if (hasCards) return <div className="space-y-2">{cards.map((card, i) => <MessageCard key={i} data={card} brut={isBrut} />)}</div>;
                        ...
                        <div className="px-4 pt-3 shrink-0 sm:pb-4"
                            style={{ borderTop: `${isBrut ? 2 : 1}px solid var(--chat-line)`, paddingBottom: "max(1rem, env(safe-area-inset-bottom, 1rem))" }}>
                            <GlowingChatInput
                                value={input}
