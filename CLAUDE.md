...
**Fonts (what's actually loaded):** **Manrope** for body + all UI (`--font-inter` / `font-sans`, also aliased to `--font-brut-mono` / `--font-raleway`) and **EB Garamond** (a classic literary serif, normal + italic) for headings + big display titles (`--font-brut-display` / `font-display`). The variable names (`--font-inter`, `--font-brut-display`) are kept for continuity even though the families changed; older `"Space Grotesk"` / `"Sora"` / `"JetBrains Mono"` mentions are stale. A semantic type-scale token system lives in `globals.css` under `.theme-brut` (`--text-*`, `--lh-*`, `--ls-*`). Trust the loaded fonts over any leftover names.
